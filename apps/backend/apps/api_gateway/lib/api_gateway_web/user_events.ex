defmodule ApiGatewayWeb.UserEvents do
  ...
  def avatar_changed(actor_id, avatar_media_id)
    ...
    # and echoing it back would make every client that listens re-fetch its own card for nothing.
    |> Enum.reject(&(&1 == actor_id))
    |> Enum.each(&ApiGatewayWeb.Endpoint.broadcast("user:" <> &1, "user_updated", payload))

    :ok
  rescue
    error ->
      Logger.warning("user_updated fan-out skipped for #{actor_id}: #{inspect(error)}")
      :ok
  end
  ...
  defp peers_of(actor_id) do
    case SharedInfra.ConversationClient.peers_of(%{"user_id" => actor_id}) do
      {:ok, result} ->
        Map.get(result, :user_ids) || Map.get(result, "user_ids") || []
