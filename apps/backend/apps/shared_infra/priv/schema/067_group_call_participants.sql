-- Feature: Phase-3 group calling — per-participant membership of a GROUP call (Slice A1, data foundation).
...
--
-- NAMED `group_call_participants` — NOT `call_participants` — because the LEGACY `call_participants` table
-- (migration 010, the abandoned native-WebRTC design keyed by call_session_id) already exists on live DBs.
-- Reusing that name collides (CREATE TABLE IF NOT EXISTS silently skips → wrong columns). This table is
-- brand new and unrelated to the 010 tables.
--
...
--     psql -U chat_user -d chat_platform -f - < .../067_group_call_participants.sql
BEGIN;
...
CREATE INDEX IF NOT EXISTS idx_group_call_participants_user
  ON group_call_participants(user_id, created_at DESC);
