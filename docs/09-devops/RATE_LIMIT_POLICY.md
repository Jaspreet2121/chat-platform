...
| `POST /nearby/ble/token` | 4 | 60s | user | **CLOSED** | BLE broadcast-token issuance (104). Tokens live 300s and re-request rotates the old one out, so a handset needs at most ~1/min; more is churn or abuse. Closed like every nearby surface. |
| `POST /nearby/ble/sightings` | 12 | 60s | user | **CLOSED** | BLE sighting reports (104, ≤20 tokens/call). The response is a count only, but resolution work and marker writes are server cost — closed keeps an outage from opening a probing loop. |
| `GET /dating/deck` | 12 | 60s | user | **CLOSED** | The deck returns up to 25 identity cards with photos — the widest dating enumeration surface. Closed, the nearby-discover precedent. |
| `POST /dating/swipes` | 60 | 60s | user | **CLOSED** | Swiping is fast by design (60/min); still closed — a like reveals intent to the target and mints matches, so a limiter outage must not unbound it. |
| `POST /keys/device` | 10 | 60s | user | **CLOSED** | Device public-key upload/rotation (107). A device rotates rarely; a flood is abuse. Key distribution is a correctness surface for E2E clients, so an outage rejects rather than accepting writes it can't limit. |
| `GET /keys/users` | 30 | 60s | user | **CLOSED** | Membership-gated key fetch (107, ≤50 ids/call). The gate is the control against enumeration; the limiter bounds how fast anyone can walk it — closed, the contacts-sync precedent. |
