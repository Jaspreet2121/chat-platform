...
Kafka pipe is end-to-end for `message.created.v1`: brod-backed producer (flag-gated, async) → broker → two consumers — minimal `MessageCreatedLogConsumer` (log/ack) and the first stateful `ConversationSummaryConsumer` (idempotent `conversation_message_summaries` projection via the `processed_events` ledger, exactly-once, poison-skip); all verified live + exactly-once unit proof. Next Kafka step: the **notification-service reactive consumer** (fanout/notifications), which copies the `ConversationSummary` dedupe pattern verbatim. Other open work: PG message store enablement in dev/prod (Repo not auto-started); message-list pagination; block-state/tenant authz + `Permissions.authorize/1` placeholder; ScyllaDB live backend (Phase 8). CI now installs cmake for the brod NIF.
