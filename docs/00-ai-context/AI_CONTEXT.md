...
Next steps: **recipient fan-out** for notification-service (one record per conversation participant — needs ConversationService participant data, cross-service), OR the **next missing service** (tenant/call-signaling/moderation/audit). Other open work: PG message store enablement in dev/prod (Repo not auto-started); message-list pagination; block-state/tenant authz + `Permissions.authorize/1` placeholder; ScyllaDB live backend (Phase 8). CI now installs cmake for the brod NIF.
