...
**Deployment foundation started (2026-06-18, sub-slice 1):** the project can now boot as a real server. Repos are supervised at boot in dev/prod (gated `start_repo: false` in `:test` so plain `mix test` stays Docker-free — fixes the prior `children = []` "never booted" gap). `config/runtime.exs` fails fast in `:prod` via `SharedInfra.ProdConfig` if `SECRET_KEY_BASE`/`TOKEN_SECRET`/`OTP_SECRET` are missing/placeholder (audit #4 mitigated); `config/prod.exs` + a `mix release` (`chat_platform`, all 8 apps) added. Hosting plan: Fly + managed Postgres + Vercel web, Kafka staged OFF first (see `docs/09-devops/DEPLOYMENT.md`). **Sub-slice 2 DONE (2026-06-18):** multi-stage [apps/backend/Dockerfile](apps/backend/Dockerfile) (build `elixir:1.18.4-otp-27` + cmake for brod; runtime `debian:bookworm-slim`, ERTS bundled, non-root) + `.dockerignore`; image builds (≈262 MB) and the prod guard fires IN-container (no secrets → `FATAL: DATABASE_URL is not set`; placeholder → rejected; valid dummy secrets → boots). No secrets baked in. **Sub-slice 3a DONE (2026-06-18):** [apps/backend/fly.toml](apps/backend/fly.toml) (deploy from apps/backend, `internal_port 4000`, force_https, `min_machines_running 1` for WS, shared-cpu-1x/512MB, core-chat-ON/Kafka-OFF `[env]`, no secrets) + a full ordered deploy runbook + debug-loop table in `docs/09-devops/DEPLOYMENT.md`. Schema apply = manual ordered psql (`001→042`) once (no Ecto migrations; init SQL is outside the release image). `runtime.exs` confirmed to read every Fly boot value (no code change). NOT yet done: **sub-slice 3b = the user's actual `fly deploy`** (flyctl not installed here — deploy + managed-PG TLS + WSS are the real, unverifiable-here proof), then baseline observability (4 — structured logs + real correlation IDs replacing `corr_placeholder`).

**Microservices split IN PROGRESS (2026-06-18, sub-slice 1 of ~12-18).** Firm decision: split the umbrella into separately-deployable service containers (network comms, not in-process). Inspection found all cross-app coupling is at the two EDGE apps (api_gateway, realtime_gateway) — services are already event-decoupled. **Sub-slice 1 DONE — the service-client boundary pattern, applied to Auth:** `SharedInfra.AuthClient` (behaviour + dispatcher, mirrors `Kafka.Producer`) selects its adapter from `:shared_infra, :auth_client_adapter`; default `AuthService.AuthClientInProcess` delegates in-process (zero behavior change). Edge apps now call `SharedInfra.AuthClient.*` instead of `AuthService.*` (grep-confirmed none remain). A future `AUTH_CLIENT_ADAPTER=http` adapter drops in without touching call sites. **Sub-slice 2 DONE (Conversation):** `SharedInfra.ConversationClient` + `ConversationService.ConversationClientInProcess` (default in-process); edge apps (conversation_controller, message_controller membership check, realtime topic_authorization) call `SharedInfra.ConversationClient.{create/list/get_conversation,add/remove_participant}` — no `ConversationService.*` left. Zero behavior change (203→205 plain, 271→273 pg). **Sub-slice 3 DONE (User):** `SharedInfra.UserClient` + `UserService.UserClientInProcess` (default in-process); api_gateway/user_controller calls `SharedInfra.UserClient.{get_current_profile,get_public_profile,update_current_profile}` — no `UserService.*` left. Zero behavior change (205→207 plain, 273→275 pg). **Sub-slice 4 DONE (Message, heaviest seam):** `SharedInfra.MessageClient` + `MessageService.MessageClientInProcess` (default in-process); both edges (api_gateway/message_controller + realtime_gateway/conversation_channel, 15 call-sites) call `SharedInfra.MessageClient.*` — no `MessageService.{Messages,Timeline,Receipts}.*` left (`list_timeline` maps to `Timeline.list_messages`). Zero behavior change (207→211 plain, 275→279 pg). **Sub-slice 5 DONE (Media) — CLIENT-BOUNDARY SET COMPLETE:** `SharedInfra.MediaClient` + `MediaService.MediaClientInProcess` (default in-process); api_gateway/media_controller calls `SharedInfra.MediaClient.{create_upload,complete_upload,get_download_url}` — no `MediaService.Media.*` left. Zero behavior change (211→215 plain, 279→283 pg). **All five edge→service seams (Auth/Conversation/User/Message/Media) now route through `SharedInfra.*Client` dispatchers** — no edge app calls any `*Service.*` domain module directly.

**Internal HTTP API phase STARTED — Auth template DONE (2026-06-18):** each service now needs an internal HTTP API so future HTTP client adapters can call over the network. Built `SharedInfra.InternalApi` (the shared result-envelope `{:ok,map}⇄{"ok":map}`, `{:error,atom}⇄{"error":"name"}`, bare⇄`{"result":v}`, with `String.to_existing_atom` key rehydration — preserves the error atoms the gateway matches on; `current_session` atom-key set documented) + `SharedInfra.InternalApi.TokenPlug` (internal `x-internal-token` auth, fails closed — NEW security surface) + `AuthService.HTTP.Router` (Plug, not Phoenix; routes 1:1 with `SharedInfra.AuthClient`). Listener is a `Plug.Cowboy` child gated by `AUTH_HTTP_API_ENABLED` (default off → no listener at boot, `AuthService.Supervisor` children `== []` in test). Nothing calls it yet (in-process adapters stay default) → zero behavior change (215→227 plain, 283→295 pg). Contract: `docs/09-devops/INTERNAL_API.md`. **Conversation internal API DONE (2026-06-18):** `ConversationService.HTTP.Router` (Plug, gated `CONVERSATION_HTTP_API_ENABLED`, default off) — routes 1:1 with `SharedInfra.ConversationClient`; atom-keyed conversation/participant responses round-trip via the shared envelope; error atoms preserved; zero behavior change (227→231 plain, 295→299 pg). **User internal API DONE (2026-06-18):** `UserService.HTTP.Router` (gated `USER_HTTP_API_ENABLED`, default off; profile responses atom-keyed `user_id/display_name/avatar_media_id/bio`, `:profile_invalid` preserved; 231→235 plain, 299→303 pg). **Message internal API DONE (2026-06-18, heaviest 9 routes):** `MessageService.HTTP.Router` (gated `MESSAGE_HTTP_API_ENABLED`, default off; `list_timeline`→`Timeline.list_messages`; error atoms `:message_invalid`/`:message_forbidden`; documented metadata-string-key fidelity caveat for the future client adapter; 235→240 plain, 303→308 pg). **Media internal API DONE (2026-06-18) — INTERNAL-API SET COMPLETE (all 5):** `MediaService.HTTP.Router` (gated `MEDIA_HTTP_API_ENABLED`, default off; listener is media's only child — no Repo; 240→244 plain, 308→312 pg). All 5 services (Auth/Conversation/User/Message/Media) now expose internal HTTP APIs (Plug, reusing `SharedInfra.InternalApi` + `TokenPlug`), gated default-off, uniform wire contract (INTERNAL_API.md). **HTTP client adapter phase STARTED — Auth adapter DONE (2026-06-23):** `SharedInfra.HttpClient` (shared helper: build req + `x-internal-token` + JSON, 2s/5s timeouts, HTTP 200→`decode_result`, transport failure→`{:error, unavailable_atom}`) + `SharedInfra.AuthClientHttp` (lives in shared_infra, NOT auth_service; `:auth_unavailable`; `persistence_enabled?` fails closed). Flip via `AUTH_CLIENT_ADAPTER=http` + `AUTH_SERVICE_URL`; default stays in-process (zero behavior change). **HTTP lib = `:httpc` not Req** — the package registry is unreachable in this env so Req couldn't be fetched; `:httpc` (zero-dep) is used, isolated in `HttpClient` so Req can be swapped later. Gateway maps `{:error, :auth_unavailable}`→**503** (`ErrorResponse.service_unavailable/2` + a clause at every `current_session` site + auth otp/refresh/logout — additive, dead in the default path). Verified: 244→246 plain (+2 mapping tests), 312→314 pg, `--include http_integration` 3 passed (real round-trip == in-process). **Conversation HTTP adapter DONE (2026-06-23):** `SharedInfra.ConversationClientHttp` (flip `CONVERSATION_CLIENT_ADAPTER=http`+`CONVERSATION_SERVICE_URL`; default in-process); gateway/realtime map `:conversation_unavailable`→503/realtime.unavailable at every call-site (conversation_controller, message membership gate — `authorize_membership` now propagates it instead of 403, realtime topic_authorization). 246→248 plain, 314→316 pg, conversation `--include http_integration` 3 passed. **User HTTP adapter DONE (2026-06-23):** `SharedInfra.UserClientHttp` (flip `USER_CLIENT_ADAPTER=http`+`USER_SERVICE_URL`; default in-process); gateway maps `:user_unavailable`→503 at every user_controller site (incl. fixing the public-`profile` no-catch-all crash). 248→249 plain, 316→317 pg, user http_integration 3 passed. **Message HTTP adapter DONE (2026-06-23):** `SharedInfra.MessageClientHttp` (9 callbacks, flip `MESSAGE_CLIENT_ADAPTER=http`); the metadata caveat resolved via `decode_result/2` `skip_atomize: ["metadata"]` (free-form metadata stays string-keyed at any depth; other adapters unchanged); gateway→503 + realtime→`realtime.unavailable`. 249→254 plain, 317→322 pg, message http_integration 4 passed + decode skip unit tests. **Media HTTP adapter DONE (2026-06-23) — CLIENT-ADAPTER SET COMPLETE (all 5):** `SharedInfra.MediaClientHttp` (flip `MEDIA_CLIENT_ADAPTER=http`); gateway→503. 254→255 plain, 322→323 pg, media http_integration 3 passed. **All 5 `SharedInfra.*Client`s (Auth/Conversation/User/Message/Media) now flip to HTTP behind a per-service `*_CLIENT_ADAPTER=http` flag + `*_SERVICE_URL`**, with transport-failure→`:*_unavailable`→503/realtime-reject and proven shape-identical round-trips; defaults all in-process (zero behavior change). **Per-service releases DONE (2026-06-23, packaging only — zero runtime change):** Phase-1 inspection decided mechanism (iii) — NO shared_infra package extraction needed (shared_infra has ZERO compile-coupling to services). Instead: dropped the now-unused `{:*_service, in_umbrella}` deps from `api_gateway`/`realtime_gateway` mix.exs (all calls go via `SharedInfra.*Client` — the real decoupling that keeps the gateway image lean) + added per-service `mix release` definitions to `apps/backend/mix.exs` (auth/user/conversation/message/media each `[<svc>, shared_infra]`; `gateway` = `[api_gateway, realtime_gateway, shared_infra]`; kept all-in-one `chat_platform`). KEY PROOF the dep-drop is runtime-safe: `mix test` 255/89 + pg 323 UNCHANGED (in-process adapters still resolve at runtime — the dep only governed compile-order/bundling). All 7 releases assemble (`MIX_ENV=prod mix release`); bundling confirmed lean (gateway/lib has NO service apps; auth_service/lib = auth_service + shared_infra only). NEXT: per-service Dockerfiles → docker-compose.prod → optional DB-per-service → CI rework. NOTE: deploy sub-slice 3b (run the single container) + notification_service per-service release still pending.
