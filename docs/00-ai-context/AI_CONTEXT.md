...
- **PostgreSQL is the source of truth for message timelines.** [VERIFIED 2026-08-05]
  `MESSAGE_STORE_ADAPTER=postgres` ([docker-compose.prod.yml:153](../../docker-compose.prod.yml#L153)).
  - ~~ScyllaDB is the source of truth for message timelines.~~ **← FALSE AS A STATEMENT OF FACT.**
    It was an architecture *intent* written in the present tense, and it read as status. The Scylla
    adapters exist and are CI-tested (7 scylla-gated suites, 0 excluded, on every push), but they
    have **never run in production**: `git log -S 'scylla_read' -- docker-compose.prod.yml` returns
    **nothing** — that string has never been in that file.
