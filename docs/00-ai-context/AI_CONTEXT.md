...
Other open work: PG message store enablement in dev/prod (Repo not auto-started); message-list pagination; block-state/tenant authz + `Permissions.authorize/1` placeholder; ScyllaDB live backend (Phase 8); the 4 remaining missing services (tenant/call-signaling/moderation/audit). CI installs cmake for the brod NIF.
