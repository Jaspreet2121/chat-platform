...
Next steps: **(b)** notification-service consumes those participant events into a LOCAL participant read-model (idempotent, flag-gated — reuses the dedupe blueprint), then **(c)** fan-out: on `message.created.v1`, read the local participant set and insert one notification per recipient (excluding `sender_user_id`), idempotent per `(source_event_id, recipient_user_id)`. Cold-start/ordering race (message before read-model populated → notifies nobody) accepted + to be documented. Other open work: PG message store enablement in dev/prod (Repo not auto-started); message-list pagination; block-state/tenant authz + `Permissions.authorize/1` placeholder; ScyllaDB live backend (Phase 8). CI installs cmake for the brod NIF.
