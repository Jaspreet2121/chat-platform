...
**Sub-slice (b) DONE (2026-06-18):** notification-service consumes the participant events into a LOCAL read-model. `NotificationService.ParticipantReadModel.apply_participant_added/1` + `apply_participant_removed/1` maintain `conversation_participants_readmodel` (soft-state `active` boolean, NEVER hard-deleted) with TWO correctness mechanisms — dedupe via `notification_processed_events` (distinct consumer `notification-participants`) for same-event redelivery, AND occurred_at **last-writer-wins** (update only when `occurred_at >= last_event_at`) for out-of-order convergence (the producer's per-event `Task` means produce order ≠ logical order). New `ConversationParticipantsConsumer` (topic `conversation.events.v1`, distinct group, flag `NOTIFICATION_PARTICIPANTS_CONSUMER_ENABLED`, default off) reuses the existing brod client. Convergence proven incl. the out-of-order case (deliver newer remove before older add → stays inactive).

Next step: **(c) fan-out** — on `message.created.v1`, read the active set from `conversation_participants_readmodel` (`WHERE conversation_id=? AND active`), insert one notification per recipient EXCLUDING `sender_user_id`, idempotent per `(source_event_id, recipient_user_id)` (a new unique index + `on_conflict: :nothing`). Cold-start/ordering race (message before read-model populated → notifies nobody) accepted + to be documented. Other open work: PG message store enablement in dev/prod (Repo not auto-started); message-list pagination; block-state/tenant authz + `Permissions.authorize/1` placeholder; ScyllaDB live backend (Phase 8). CI installs cmake for the brod NIF.
