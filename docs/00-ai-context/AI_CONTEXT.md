...
Message durability now runs on Postgres (`PostgresAdapter`). Open: wiring the message store on by default in dev/prod (Repo not auto-started + flag off by default); message-list pagination/cursors; block-state/tenant authz + `Permissions.authorize/1` placeholder; ScyllaDB live backend (Phase 8); Kafka events (unwired). Candidate next slices: dev/prod enablement of the PG message store, message-list pagination, or block-list/tenant authz.
