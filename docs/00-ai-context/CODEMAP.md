...
- `apps/backend/apps/shared_infra/lib/shared_infra/message_client_http.ex` (`SharedInfra.MessageClientHttp` — HTTP adapter for `SharedInfra.MessageClient`, 9 callbacks; flip via `MESSAGE_CLIENT_ADAPTER=http`+`MESSAGE_SERVICE_URL`; decodes with `skip_atomize: ["metadata"]` so free-form metadata stays string-keyed; transport failure→`:message_unavailable`→503/realtime.unavailable). `SharedInfra.InternalApi.decode_result/2` gained the `skip_atomize` opt; `SharedInfra.HttpClient` threads `decode:` opts.
- `apps/backend/apps/shared_infra/lib/shared_infra/media_client_http.ex` (`SharedInfra.MediaClientHttp` — HTTP adapter for `SharedInfra.MediaClient`; flip via `MEDIA_CLIENT_ADAPTER=http`+`MEDIA_SERVICE_URL`; transport failure→`:media_unavailable`→503). **CLIENT-ADAPTER SET COMPLETE: all 5 `SharedInfra.*Client`s flip to HTTP behind a per-service flag.**
