...
- [ ] Kafka event production/consumption — IN PROGRESS: `message.created.v1` flows end-to-end producer→broker→consumer. Producer: `SharedInfra.Kafka.BrodProducer` (async, `:hash`, fire-and-forget, flag-gated, default `NoopProducer`). Consumer: minimal `MessageService.Events.MessageCreatedLogConsumer` (`brod_group_subscriber_v2`, `KAFKA_CONSUMER_ENABLED`, log/ack only). Pipe verified live (`--include kafka_integration`). Still pending: a **real reactive consumer** (fanout/notifications — must dedupe on `event_id`) and the remaining ~40 catalog events
