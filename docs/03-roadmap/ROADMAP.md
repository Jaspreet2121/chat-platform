...
- [ ] **CI rework Layer 3** — compose-integration job: `docker compose -f docker-compose.prod.yml up` + the gateway→auth 401/503/recover curl differential, **label/nightly-gated** (heavy image build; must not slow PRs).
