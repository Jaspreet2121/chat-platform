...
- [ ] Recipient fan-out (event-driven cross-service data, NOT sync calls) — **sub-slice (a) DONE (2026-06-18):** ConversationService produces `conversation.participant_added.v1`/`participant_removed.v1` (fire-and-forget, flag `CONVERSATION_PUBLISH_ENABLED`, default off; `BrodProducer` client now per-call configurable). Pending: **(b)** notification-service participant read-model consumer, **(c)** fan-out one notification per recipient (excl. sender), idempotent per `(source_event_id, recipient_user_id)`. See DECISION_LOG 2026-06-18.
