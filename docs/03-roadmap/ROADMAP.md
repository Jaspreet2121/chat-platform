...
- [ ] **DEFERRED — Kafka consumer correlation→metadata regression guards** (observability review findings 1 & 2): assert each of the 4 consumers extracts `envelope["correlation_id"]` into Logger metadata. Needs the consumers to echo the id through their test-notification (~5 edits) + assertions in the `kafka_integration` tests (live broker only). **Fold into the Kafka/MinIO staged-re-enable + notification_service-container slice.** (The extraction itself is the unit-tested `SharedInfra.Correlation.put/1`.)
