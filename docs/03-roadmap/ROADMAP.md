...
- [ ] **deploy 3b** — run a stack on a real host (Fly/VM or self-host compose), beyond local compose. **Both code blockers now closed** (schema-load + OTP delivery). Remaining = host config only: `apps/backend/fly.toml` ready (set `app`/`PHX_HOST`, `fly secrets set`, `fly postgres attach`); enable `OTP_SMS_DELIVERY_ENABLED` + the SMS secrets; set `WEB_ORIGIN` once the web URL is known; deploy `apps/web` separately with the API/WS URLs. Self-host compose: see the self-hosted checklist (PHX_HOST edit, optional TLS proxy, `.env` secrets, ≥4 GB RAM).
