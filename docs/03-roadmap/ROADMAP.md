...
- [ ] **MinIO into compose (own slice):** add `minio` + a `minio-init` (create the `chat-media` bucket) + `MEDIA_STORAGE_ADAPTER=minio` + MinIO env on the media container; prove a real upload/download presign through the media container.
...
- [ ] **deploy 3b** — run a stack on a real host (Fly/VM), beyond local compose.
