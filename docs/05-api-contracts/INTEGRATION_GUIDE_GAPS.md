...
**Run status: the walk has not yet run against production.** Lines 1–2 were found by reading the
guide as an outsider during the walk's design review; line 3 by a live probe (2026-08-08); line 4 by
reading the ownership rule while building the walk (the walk asserts it in step 8 when run). A
zero-gap claim for any guide section is earned only by a passing walk of that section — none is
claimed yet.
...
- The guide's step-4 response shows a `message_id` field. The walk asserts the real key at runtime
  and emits a conformance note if reality spells it differently; unconfirmed until the first run.

## Coverage map (guide section → walk step)

| Guide | Walk step | Walked? |
|---|---|---|
| §2 step 1 — register an app | 1 (distinct `app_id`, not tenant-zero, owner row via `GET /api/v1/apps`) | not yet |
| §2 step 2 — issue a key | 2 (`mode:"test"` → `sk_test_` prefix, once-only key, **twin app_id minted**), 2b (literal live key) | not yet |
| §2 step 3 — create a conversation | 3 (secret-key server actor) | not yet |
| §2 step 4 — send a message | 4 (REST send, key names the sender; response-shape pin) | not yet |
| §2 step 5 — register a webhook | 5 (`/v1` + `sk_test_`, `whsec_` once, never on the list), 5b (the session-scoped `/api/v1` note, owned `app_id`) | not yet |
| §4 token exchange | 6/7 (every end-user token in the walk is minted via `POST /v1/auth/token`) | not yet |
| §6 socket | 4 (the other participant's SDK socket receives the sent message, same id) | not yet |
| §7–§8 webhook delivery + signatures | 8 (attempt + status in the delivery log — evidence floor; signature *verification* needs a real receiver: out of scope, see gap 3) | not yet |
| §9 test vs live | 2 (twin mint), 7 (isolation both directions), 8 (twin delivery-log 403 — gap 4) | not yet |
| tenant isolation (implicit contract) | 6 (foreign conversation ids are 404, byte-identical to a nonexistent id) | not yet |
| event pipeline health (not in the guide) | 9 (admin outbox summary: staged=0/pending=0 — zeros mean health) | not yet |
