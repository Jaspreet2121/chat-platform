...
| 4 | §9: test-twin webhooks are registered and managed with the `sk_test_` key on `/v1` — presented as a complete test-mode story. | ~~The **delivery log** rides the owner console, and ownership is `app_owners` rows — which a test twin never has, so `app_id=<twin>` answers **403**: twin deliveries are dispatched but **unobservable by their integrator**.~~ **FIXED 2026-08-15** — `fix(webhooks): let app owners read their twin's delivery log`: `owns_app` now traverses `parent_app_id + mode='test'` exactly one hop downward, so owning the live app grants its twin across the whole owner console (deliveries, endpoints, keys, usage). The walk's step 8 asserts the twin log is **reachable with evidence** from that commit on. Was: High. | **Fixed — verified in production 2026-08-15**: the post-deploy walk ran 19/19 with step 8 green (twin delivery log 200, attempt visible with its status to the owner) |
...
| §9 test vs live | 2 (twin mint), 7 (isolation both directions), 8 (twin delivery log — pinned the gap-4 403 on 2026-08-09; asserts reachable-with-evidence since the fix) | **walked 2026-08-15** — post-deploy run 19/19, step 8 green: the twin's delivery attempt is visible to its owner |
