...
33. **`restart: unless-stopped` means exactly that: a container YOU stopped stays stopped across a reboot.** The 2026-09-23 night window ran `docker compose stop --timeout 60` before `sudo reboot`, to get a clean shutdown. That call marks every container user-stopped, and `unless-stopped` deliberately does NOT bring those back when the daemon starts — so the box rebooted into an empty stack. **Prod was down 01:46 → 11:15 IST (~9.5 h)**, until a manual `docker compose start`. Three separate things had to be absent for that: the runbook's own post-reboot check said "every service `Up`" but was never run (the operator had gone to bed), there was no external uptime alert, and the 21:30 UTC backup ran against a stopped Postgres and wrote a 20-byte file with no error anywhere. **The lessons: never stop the stack before a reboot** — raise the daemon's `shutdown-timeout` instead so a plain `sudo reboot` is graceful (`unless-stopped` then restores everything, because the daemon stopped them, not you); **a runbook step nobody is awake to read is not a check** — it needs an external monitor; and **every unattended job must be able to say it failed** (see `scripts/ops/pg-backup.sh`).
